...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!record) return withCors({ error: "No OTP found", code: "OTP_NOT_FOUND" }, 404);

    if (record.expiresAt < new Date()) return withCors({ error: "OTP expired", code: "OTP_EXPIRED" }, 400);
    if (record.code !== otp) return withCors({ error: "Invalid OTP", code: "INVALID_OTP" }, 400);

    await db.thirdPartyOTP.delete({ where: { email } });

    const res = withCors({ success: true, message: "OTP verified successfully" }, 200);
    res.headers.set("X-RateLimit-Limit", limit.toString());
    res.headers.set("X-RateLimit-Remaining", remaining.toString());
    res.headers.set("X-RateLimit-Reset", reset.toString());
    return res;
