  ...
  try {
    ...
    let notificationTemplate = await db.newsletterOwnerNotification.findFirst({
      where: {
        ...
        category: NewsletterOwnerNotificationCategory.KYC_APPROVAL,
    ...
    // 7. Create system notification for the user
    await createSystemNotification(kycApplication, userName);
...
function personalizeKycContent(
  ...
  return {
    ...
    html: (template.html || '')
      ...
      .replace(/\[Reviewer Name\]/g, kycApplication.reviewedBy || 'System')
...
async function createSystemNotification(kycApplication: any, userName: string) {
  const message = kycApplication.status === 'APPROVED'
    ? `Your KYC application has been approved by ${kycApplication.reviewedBy || 'an administrator'}.`
    : `Your KYC application requires additional review. Status: ${kycApplication.status}`;

  await db.systemNotification.create({
    data: {
      type: NotificationType.SYSTEM,
      title: `KYC Application ${kycApplication.status}`,
      content: message,
      status: NotificationStatus.SENT,
      category: SystemNotificationCategory.KYC_APPROVAL,
      priority: kycApplication.status === 'APPROVED' 
        ? NotificationPriority.HIGH 
        : NotificationPriority.MEDIUM,
      metadata: {
