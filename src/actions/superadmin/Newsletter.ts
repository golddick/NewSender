...
import { TopNewsletter } from "@/app/configs/types";
...
//   }
  ...
  // Fetch stats
  const totalOwners = await db.membership.count({
    where: {
      OR: [
        { emails: { some: {} } },
        {
  ...
  const premiumOwners = await db.membership.count({
    where: { plan: { not: "FREE" } },
  });

  const totalSubscribers = await db.subscriber.count();

  const ownersWithSubscribers = await db.subscriber.groupBy({
  ...
  const topOwners = await db.membership.findMany({
    where: {
      userId: {
...
const topPerformingNewsletters: TopNewsletter[] = topOwners
    .map((owner) => {
      ...
      const totalSent = owner.emails.reduce((sum, e) => sum + (e.emailsSent || 0), 0);
      ...
      return {
        name: owner.emails.length > 0 ? owner.emails[0].title : "Untitled Newsletter",
