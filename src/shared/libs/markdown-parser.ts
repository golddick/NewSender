...
//       .replace(
//         /^### (.*$)/gm,
//         '<h3 class="text-lg font-semibold mt-6 mb-3 text-gray-900 border-l-4 border-blue-500 pl-4">$1</h3>',
//       )
//       .replace(
//         /^## (.*$)/gm,
//         '<h2 class="text-xl font-bold mt-8 mb-4 text-gray-900 border-l-4 border-blue-500 pl-4 bg-blue-50 py-2 rounded-r-lg">$1</h2>',
//       )
//       .replace(
//         /^# (.*$)/gm,
//         '<h1 class="text-2xl font-bold mt-10 mb-6 text-gray-900 border-l-6 border-blue-600 pl-6 bg-gradient-to-r from-blue-50 to-transparent py-3 rounded-r-lg">$1</h1>',
//       )
...
//       .replace(
//         /^&gt; (.*$)/gm,
//         '<blockquote class="border-l-4 border-blue-500 pl-6 py-2 my-4 bg-blue-50 italic text-gray-700 rounded-r-lg">$1</blockquote>',
//       )
...
//       .replace(/^(?!<[h|l|b|p|d])(.+)$/gm, '<p class="mb-4 text-gray-700 leading-relaxed">$1</p>')
...
export const parseMarkdown = (text: string): string => {
  ...
  processedText = processedText.replace(
    ...
    (match) => {
      ...
      return `<blockquote class="border-l-4 border-blue-500 pl-6 py-2 my-4 bg-blue-50 italic text-gray-700 rounded-r-lg">${content}</blockquote>`;
    }
  );
...
// Process links with new tab opening for external URLs
  ...
  (match, text, url) => {
    ...
    return `<a href="${finalUrl}" class="text-blue-600 hover:text-blue-800 underline font-medium" ${attrs}>${text}</a>`;
  }
);
